...
class CNTGenerator:
    ...
    def generate_armchair_cnt(self):
        ...
        molecular_model.name = f"Armchair CNT (N={self.n}, h={self.z_length})"
        ...
        # Create bonds
        self.create_cnt_bonds(molecular_model, atoms, atom_grid, num_z_cells)

        return molecular_model, atoms, radius

    def create_cnt_bonds(self, molecular_model, atoms, atom_grid, num_z_cells):
...
if __name__ == "__main__":
    ...
    cnt_molecule, _, _ = generator.generate_armchair_cnt()

    SAMSON.beginHolding("Create CNT Model")
    structural_model = SBStructuralModel()
    structural_model.name = f"Armchair CNT (N={n}, h={z_length})"
